...
_xor_shift_user:     file format elf32-i386
...
int main(int argc, char *argv[])
{
  ...
  14:	e8 07 02 00 00       	call   220 <atoi>
  19:	89 c3                	mov    %eax,%ebx
	b = atoi(argv[2]);
  1b:	8b 46 08             	mov    0x8(%esi),%eax
  1e:	89 04 24             	mov    %eax,(%esp)
  21:	e8 fa 01 00 00       	call   220 <atoi>
  26:	89 c6                	mov    %eax,%esi
	rand = random();
  28:	e8 dd 02 00 00       	call   30a <random>
	sc_rand = (rand%256);
	sc_rand1 = a + sc_rand;
	printf(1,"\nA IS %d and B is %d and The random number is %d\t and the random number is %d\n",a,b,sc_rand1,rand);
  2d:	89 74 24 0c          	mov    %esi,0xc(%esp)
  31:	89 5c 24 08          	mov    %ebx,0x8(%esp)
  35:	c7 44 24 04 48 07 00 	movl   $0x748,0x4(%esp)
  3c:	00 
  ...
  44:	0f b6 d0             	movzbl %al,%edx
	sc_rand1 = a + sc_rand;
	printf(1,"\nA IS %d and B is %d and The random number is %d\t and the random number is %d\n",a,b,sc_rand1,rand);
  47:	89 54 24 14          	mov    %edx,0x14(%esp)
	int a,b;
	a = atoi(argv[1]);
	b = atoi(argv[2]);
	rand = random();
  ...
  4b:	01 da                	add    %ebx,%edx
	printf(1,"\nA IS %d and B is %d and The random number is %d\t and the random number is %d\n",a,b,sc_rand1,rand);
  4d:	89 54 24 10          	mov    %edx,0x10(%esp)
  51:	e8 8a 03 00 00       	call   3e0 <printf>
	exit();
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 379:	0f b6 92 9f 07 00 00 	movzbl 0x79f(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 50f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 511:	b8 98 07 00 00       	mov    $0x798,%eax
...
free(void *ap)
{
 ...
 63f:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 642:	a3 18 0a 00 00       	mov    %eax,0xa18
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 669:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 66c:	8b 1d 18 0a 00 00    	mov    0xa18,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 6a9:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 6ab:	3b 15 18 0a 00 00    	cmp    0xa18,%edx
...
morecore(uint nu)
{
 ...
 6ca:	e8 43 fc ff ff       	call   312 <sbrk>
  if(p == (char*)-1)
 ...
 6dd:	e8 ee fe ff ff       	call   5d0 <free>
  return freep;
 6e2:	8b 15 18 0a 00 00    	mov    0xa18,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 71c:	eb e9                	jmp    707 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 71e:	c7 05 18 0a 00 00 1c 	movl   $0xa1c,0xa18
 725:	0a 00 00 
    base.s.size = 0;
 728:	ba 1c 0a 00 00       	mov    $0xa1c,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 72d:	c7 05 1c 0a 00 00 1c 	movl   $0xa1c,0xa1c
 734:	0a 00 00 
    base.s.size = 0;
 737:	c7 05 20 0a 00 00 00 	movl   $0x0,0xa20
 73e:	00 00 00 
