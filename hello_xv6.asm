...
void hello_xv6(int argc ,char *argv[]){
   ...
   c:	c7 44 24 04 06 07 00 	movl   $0x706,0x4(%esp)
  13:	00 
  14:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  1b:	89 44 24 08          	mov    %eax,0x8(%esp)
  1f:	e8 7c 03 00 00       	call   3a0 <printf>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 339:	0f b6 92 2b 07 00 00 	movzbl 0x72b(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 4cf:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 4d1:	b8 24 07 00 00       	mov    $0x724,%eax
...
free(void *ap)
{
 ...
 5ff:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 602:	a3 a0 09 00 00       	mov    %eax,0x9a0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 629:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 62c:	8b 1d a0 09 00 00    	mov    0x9a0,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 669:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 66b:	3b 15 a0 09 00 00    	cmp    0x9a0,%edx
...
morecore(uint nu)
{
 ...
 68a:	e8 53 fc ff ff       	call   2e2 <sbrk>
  if(p == (char*)-1)
 ...
 69d:	e8 ee fe ff ff       	call   590 <free>
  return freep;
 6a2:	8b 15 a0 09 00 00    	mov    0x9a0,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 6dc:	eb e9                	jmp    6c7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6de:	c7 05 a0 09 00 00 a4 	movl   $0x9a4,0x9a0
 6e5:	09 00 00 
    base.s.size = 0;
 6e8:	ba a4 09 00 00       	mov    $0x9a4,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6ed:	c7 05 a4 09 00 00 a4 	movl   $0x9a4,0x9a4
 6f4:	09 00 00 
    base.s.size = 0;
 6f7:	c7 05 a8 09 00 00 00 	movl   $0x0,0x9a8
 6fe:	00 00 00 
