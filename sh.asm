      ...
      13:	0f 8f cd 00 00 00    	jg     e6 <main+0xe6>
{
  ...
  while((fd = open("console", O_RDWR)) >= 0){
      ...
      20:	00 
      21:	c7 04 24 6a 13 00 00 	movl   $0x136a,(%esp)
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      ...
      cc:	00 
      cd:	c7 44 24 04 72 13 00 	movl   $0x1372,0x4(%esp)
      d4:	00 
...
panic(char *s)
{
  ...
  printf(2, "%s\n", s);
     ...
     169:	c7 44 24 04 66 13 00 	movl   $0x1366,0x4(%esp)
     170:	00 
     ...
     1a1:	c3                   	ret    
{
  ...
  if(pid == -1)
    panic("fork");
     1a2:	c7 04 24 cc 12 00 00 	movl   $0x12cc,(%esp)
...
runcmd(struct cmd *cmd)
{
  ...
  case EXEC:
    ...
    printf(2, "exec %s failed\n", ecmd->argv[0]);
     ...
     24b:	c7 44 24 04 d8 12 00 	movl   $0x12d8,0x4(%esp)
     252:	00 
  ...
  default:
    panic("runcmd");
     2ae:	c7 04 24 d1 12 00 00 	movl   $0x12d1,(%esp)
  ...
  case PIPE:
    ...
    if(pipe(p) < 0)
      panic("pipe");
     2ba:	c7 04 24 f8 12 00 00 	movl   $0x12f8,(%esp)
  ...
  case REDIR:
    ...
    if(open(rcmd->file, rcmd->mode) < 0){
     ...
     2c9:	c7 44 24 04 e8 12 00 	movl   $0x12e8,0x4(%esp)
     2d0:	00 
...
parseredirs(struct cmd *cmd, char **ps, char *es)
{
  ...
  while(peek(ps, es, "<>")){
     6a0:	c7 44 24 08 1a 13 00 	movl   $0x131a,0x8(%esp)
     6a7:	00 
     ...
     6dc:	89 34 24             	mov    %esi,(%esp)
{
  ...
  while(peek(ps, es, "<>")){
    ...
    if(gettoken(ps, es, &q, &eq) != 'a')
      panic("missing file for redirection");
     763:	c7 04 24 fd 12 00 00 	movl   $0x12fd,(%esp)
...
parseexec(char **ps, char *es)
{
  ...
  if(peek(ps, es, "("))
     77f:	c7 44 24 08 1d 13 00 	movl   $0x131d,0x8(%esp)
     786:	00 
  ...
  ret = parseredirs(ret, ps, es);
  while(!peek(ps, es, "|)&;")){
     7de:	c7 44 24 08 34 13 00 	movl   $0x1334,0x8(%esp)
     7e5:	00 
  ...
  while(!peek(ps, es, "|)&;")){
    ...
    if(argc >= MAXARGS)
     ...
     82f:	75 97                	jne    7c8 <parseexec+0x58>
      panic("too many args");
     831:	c7 04 24 26 13 00 00 	movl   $0x1326,(%esp)
  ...
  while(!peek(ps, es, "|)&;")){
    ...
    if(tok != 'a')
      panic("syntax");
     87a:	c7 04 24 1f 13 00 00 	movl   $0x131f,(%esp)
...
parsepipe(char **ps, char *es)
{
  ...
  if(peek(ps, es, "|")){
     8ab:	c7 44 24 08 39 13 00 	movl   $0x1339,0x8(%esp)
     8b2:	00 
...
parseline(char **ps, char *es)
{
  ...
  cmd = parsepipe(ps, es);
  while(peek(ps, es, "&")){
     956:	c7 44 24 08 3b 13 00 	movl   $0x133b,0x8(%esp)
     95d:	00 
  ...
  if(peek(ps, es, ";")){
     96e:	c7 44 24 08 37 13 00 	movl   $0x1337,0x8(%esp)
     975:	00 
...
parseblock(char **ps, char *es)
{
  ...
  if(!peek(ps, es, "("))
     9df:	c7 44 24 08 1d 13 00 	movl   $0x131d,0x8(%esp)
     9e6:	00 
  ...
  if(!peek(ps, es, ")"))
     a1f:	c7 44 24 08 59 13 00 	movl   $0x1359,0x8(%esp)
     a26:	00 
...
parseblock(char **ps, char *es)
{
  ...
  if(!peek(ps, es, "("))
    panic("parseblock");
     a6d:	c7 04 24 3d 13 00 00 	movl   $0x133d,(%esp)
     a74:	e8 e7 f6 ff ff       	call   160 <panic>
  gettoken(ps, es, 0, 0);
  cmd = parseline(ps, es);
  if(!peek(ps, es, ")"))
    panic("syntax - missing )");
     a79:	c7 04 24 48 13 00 00 	movl   $0x1348,(%esp)
...
parsecmd(char *s)
{
  ...
  peek(&s, es, "");
     b64:	c7 44 24 08 e7 12 00 	movl   $0x12e7,0x8(%esp)
     b6b:	00 
     b6c:	89 5c 24 04          	mov    %ebx,0x4(%esp)
{
  ...
  if(s != es){
    printf(2, "leftovers: %s\n", s);
     ...
     b99:	c7 44 24 04 5b 13 00 	movl   $0x135b,0x4(%esp)
     ba0:	00 
     ba1:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
     ba8:	e8 93 03 00 00       	call   f40 <printf>
    panic("syntax");
     bad:	c7 04 24 1f 13 00 00 	movl   $0x131f,(%esp)
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    buf[i++] = digits[x % base];
     ...
     ed9:	0f b6 92 b7 13 00 00 	movzbl 0x13b7(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    106f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
    1071:	b8 b0 13 00 00       	mov    $0x13b0,%eax
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    11cc:	8b 1d c4 19 00 00    	mov    0x19c4,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    120b:	3b 15 c4 19 00 00    	cmp    0x19c4,%edx
...
morecore(uint nu)
{
  ...
  return freep;
    1242:	8b 15 c4 19 00 00    	mov    0x19c4,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    1264:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
    1267:	89 15 c4 19 00 00    	mov    %edx,0x19c4
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    127e:	c7 05 c4 19 00 00 c8 	movl   $0x19c8,0x19c4
    1285:	19 00 00 
    base.s.size = 0;
    1288:	ba c8 19 00 00       	mov    $0x19c8,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    128d:	c7 05 c8 19 00 00 c8 	movl   $0x19c8,0x19c8
    1294:	19 00 00 
    base.s.size = 0;
