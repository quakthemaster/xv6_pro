...
_rand_test:     file format elf32-i386
...
int main(int argc, char *argv[])
{
  ...
  15:	e8 06 03 00 00       	call   320 <atoi>
  1a:	89 44 24 2c          	mov    %eax,0x2c(%esp)
	maxp_time = atoi(argv[2]);
  1e:	8b 43 08             	mov    0x8(%ebx),%eax
  ...
  56:	89 5c 24 04          	mov    %ebx,0x4(%esp)
	...
	maxp_runtime = atoi(argv[4]);
	maxnum_process = atoi(argv[5]);
  5a:	89 44 24 24          	mov    %eax,0x24(%esp)
	nexttime = nextprocessbirthtime(minp_time, maxp_time );
  5e:	8b 44 24 2c          	mov    0x2c(%esp),%eax
  62:	89 04 24             	mov    %eax,(%esp)
  65:	e8 56 00 00 00       	call   c0 <nextprocessbirthtime>
	runtime = nextprocessbirthtime(minp_runtime, maxp_runtime );
  6a:	89 7c 24 04          	mov    %edi,0x4(%esp)
  6e:	89 34 24             	mov    %esi,(%esp)
	...
	maxnum_process = atoi(argv[5]);
	nexttime = nextprocessbirthtime(minp_time, maxp_time );
  71:	89 44 24 28          	mov    %eax,0x28(%esp)
	runtime = nextprocessbirthtime(minp_runtime, maxp_runtime );
  75:	e8 46 00 00 00       	call   c0 <nextprocessbirthtime>
	printf(1,"%d,%d,%d,%d,%d,%d,%d\n",minp_time,maxp_time,minp_runtime,maxp_runtime,maxnum_process,nexttime,runtime);
  7a:	8b 4c 24 28          	mov    0x28(%esp),%ecx
  7e:	8b 54 24 24          	mov    0x24(%esp),%edx
  82:	89 7c 24 14          	mov    %edi,0x14(%esp)
  86:	89 74 24 10          	mov    %esi,0x10(%esp)
  8a:	89 4c 24 1c          	mov    %ecx,0x1c(%esp)
  8e:	89 54 24 18          	mov    %edx,0x18(%esp)
  92:	89 44 24 20          	mov    %eax,0x20(%esp)
  96:	8b 44 24 2c          	mov    0x2c(%esp),%eax
  9a:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
  9e:	c7 44 24 04 46 08 00 	movl   $0x846,0x4(%esp)
  a5:	00 
  ...
  b1:	e8 2a 04 00 00       	call   4e0 <printf>
	...
	exit();
  b6:	e8 c7 02 00 00       	call   382 <exit>
  bb:	66 90                	xchg   %ax,%ax
  bd:	66 90                	xchg   %ax,%ax
  bf:	90                   	nop

000000c0 <nextprocessbirthtime>:
#include "user.h"
#include "fcntl.h"
#include "stdint.h"
...
int nextprocessbirthtime(int a, int b ){
  ...
  c7:	8b 5d 08             	mov    0x8(%ebp),%ebx
	...
	int j;
	rand = random();
  ca:	e8 3b 03 00 00       	call   40a <random>
	sc_rand = (rand*(b-a)) ;
  cf:	8b 55 0c             	mov    0xc(%ebp),%edx
  d2:	29 da                	sub    %ebx,%edx
  d4:	0f b6 c0             	movzbl %al,%eax
  d7:	0f af c2             	imul   %edx,%eax
	j = sc_rand %256;
  da:	99                   	cltd   
  db:	c1 ea 18             	shr    $0x18,%edx
  de:	8d 0c 10             	lea    (%eax,%edx,1),%ecx
  e1:	0f b6 c9             	movzbl %cl,%ecx
  ...
  e6:	29 c8                	sub    %ecx,%eax
	...
	sc_rand  = sc_rand - 256;
	if(sc_rand == 0){break;}
  e8:	3d 00 01 00 00       	cmp    $0x100,%eax
  ed:	74 19                	je     108 <nextprocessbirthtime+0x48>
  ef:	2d 00 02 00 00       	sub    $0x200,%eax
  f4:	c1 e8 08             	shr    $0x8,%eax
  f7:	83 c0 01             	add    $0x1,%eax
	} 
	sc_rand1 = a + k;
	return(sc_rand1);
}
  fa:	83 c4 04             	add    $0x4,%esp
	sc_rand = sc_rand - j;
	for(k=0;;k++){
	sc_rand  = sc_rand - 256;
	if(sc_rand == 0){break;}
 ...
 10a:	eb ee                	jmp    fa <nextprocessbirthtime+0x3a>
 10c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

00000110 <processruntime>:
	sc_rand1 = a + k;
	return(sc_rand1);
}
...
int processruntime(int a , int b){
 ...
 117:	8b 5d 08             	mov    0x8(%ebp),%ebx
	...
	int j;
	rand = random();
 11a:	e8 eb 02 00 00       	call   40a <random>
	sc_rand = (rand*(b-a)) ;
 11f:	8b 55 0c             	mov    0xc(%ebp),%edx
 122:	29 da                	sub    %ebx,%edx
 124:	0f b6 c0             	movzbl %al,%eax
 127:	0f af c2             	imul   %edx,%eax
	j = sc_rand %256;
 12a:	99                   	cltd   
 12b:	c1 ea 18             	shr    $0x18,%edx
 12e:	8d 0c 10             	lea    (%eax,%edx,1),%ecx
 131:	0f b6 c9             	movzbl %cl,%ecx
 134:	29 d1                	sub    %edx,%ecx
	sc_rand = sc_rand - j;
 136:	29 c8                	sub    %ecx,%eax
	for(k=0;;k++){
	sc_rand  = sc_rand - 256;
	if(sc_rand == 0){break;}
 138:	3d 00 01 00 00       	cmp    $0x100,%eax
 13d:	74 19                	je     158 <processruntime+0x48>
 13f:	2d 00 02 00 00       	sub    $0x200,%eax
 144:	c1 e8 08             	shr    $0x8,%eax
 147:	83 c0 01             	add    $0x1,%eax
	} 
	sc_rand1 = a + k;
	return(sc_rand1);
}
 14a:	83 c4 04             	add    $0x4,%esp
	sc_rand = sc_rand - j;
	for(k=0;;k++){
	sc_rand  = sc_rand - 256;
	if(sc_rand == 0){break;}
 ...
 15a:	eb ee                	jmp    14a <processruntime+0x3a>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 479:	0f b6 92 63 08 00 00 	movzbl 0x863(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 60f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 611:	b8 5c 08 00 00       	mov    $0x85c,%eax
...
free(void *ap)
{
 ...
 6d1:	a1 2c 0b 00 00       	mov    0xb2c,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 717:	89 10                	mov    %edx,(%eax)
  freep = p;
 719:	a3 2c 0b 00 00       	mov    %eax,0xb2c
}
 71e:	5b                   	pop    %ebx
 71f:	5e                   	pop    %esi
 720:	5f                   	pop    %edi
 ...
 73f:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 742:	a3 2c 0b 00 00       	mov    %eax,0xb2c
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 769:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 76c:	8b 1d 2c 0b 00 00    	mov    0xb2c,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 7a9:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 7ab:	3b 15 2c 0b 00 00    	cmp    0xb2c,%edx
...
morecore(uint nu)
{
 ...
 7dd:	e8 ee fe ff ff       	call   6d0 <free>
  return freep;
 7e2:	8b 15 2c 0b 00 00    	mov    0xb2c,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 804:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 807:	89 15 2c 0b 00 00    	mov    %edx,0xb2c
      return (void*)(p + 1);
 ...
 81c:	eb e9                	jmp    807 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 81e:	c7 05 2c 0b 00 00 30 	movl   $0xb30,0xb2c
 825:	0b 00 00 
    base.s.size = 0;
 828:	ba 30 0b 00 00       	mov    $0xb30,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 82d:	c7 05 30 0b 00 00 30 	movl   $0xb30,0xb30
 834:	0b 00 00 
    base.s.size = 0;
 837:	c7 05 34 0b 00 00 00 	movl   $0x0,0xb34
 83e:	00 00 00 
